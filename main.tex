...
\titlegraphic{
    \includegraphics[width=2.5cm]{unipi_logo.png}
}
...
\begin{frame}
    ...
    deeper than what it initially appears to be. \smallskip

    In order to show this, let us recall the definition of a ring and of
    a module.
...
\begin{frame}
    \begin{definition}[$R$-module]
        ...
        scalar multiplication $\cdot : R \times A \to A$ if:
        \begin{enumerate}
            \item $(A, +)$ is an abelian group,
            \item $1 a = a$, where $1$ is the unity of the ring $R$ and $a \in A$,
            \item $\lambda(\mu a) = (\lambda \mu) a$, for each $\lambda$, $\mu \in R$, and $a \in A$, 
            \item $(\lambda + \mu) a = \lambda a + \mu a$ for each $\lambda$, $\mu \in R$, and $a \in A$,
            \item $\lambda (a + b) = \lambda a + \lambda b$ for each $\lambda \in R$, and $a$, $b \in A$.
        \end{enumerate}
    \end{definition}
\end{frame}
...
% TODO: insert historical notes

\begin{frame}{The motivation behind abelian categories}
